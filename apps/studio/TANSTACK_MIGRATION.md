...
- Not migrated via this list: most `pages/api/**` routes (separate migration), `_app.tsx`, `_document.tsx`, `_error`, `pages/org/_/[[...routeSlug]].tsx`, `pages/project/_/[[...routeSlug]].tsx` (catch-alls — revisit at the end). The Mekka Auth admin proxy is mirrored at `routes/api/platform/auth-admin/$ref/$.ts` because the feature must work in both runtimes.
...
- [x] A `routes/project/$ref/auth/providers.tsx` ← `pages/project/[ref]/auth/providers.tsx` (Mekka provider body uses the shared AuthLayout shell)
...
- [x] A `routes/project/$ref/auth/templates/index.tsx` ← `pages/project/[ref]/auth/templates/index.tsx` (Mekka template body uses the shared AuthLayout shell)
