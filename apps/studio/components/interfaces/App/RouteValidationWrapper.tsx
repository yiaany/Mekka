...
export const RouteValidationWrapper = ({ children }: PropsWithChildren<{}>) => {
  if (!IS_PLATFORM) return children;
  return <PlatformRouteValidation>{children}</PlatformRouteValidation>;
};

const PlatformRouteValidation = ({ children }: PropsWithChildren<{}>) => {
  ...
  const { lastVisitedOrganization, setLastVisitedOrganization } =
   ...
   * @returns a boolean
   */
  const isExcludedRoute = excemptUrls.includes(router.pathname);

  const { isError: isErrorProject, error: projectError } =
    useProjectDetailQuery({ ref });

  const { data: organizations, isSuccess: orgsInitialized } =
  ...
  useEffect(() => {
    ...
    if (isExcludedRoute || !isLoggedIn) return;
