...
// path-browserify is CommonJS. Its named exports are unavailable to the ESM
// server bundle used by TanStack prerender, while its default export is stable.
const { dirname, relative } = path.posix
const common = (paths: string[]) => {
  if (paths.length === 0) return ''

  const [firstPath, ...remainingPaths] = paths.map((path) => path.split('/'))
  const commonSegments = firstPath.filter((segment, index) =>
    remainingPaths.every((path) => path[index] === segment)
  )
