...
import {
  ...
  QueryBlock,
} from "@/components/ui/QueryBlock/QueryBlock";
...
export const ReportBlock = ({
  ...
  const { sql: formattedSql, appendAutoLimit } = applyAutoLimit(
    acceptedSql,
    autoLimit,
  );
  ...
  const {
    ...
    queryFn: async () => {
      ...
      if (!connectionString) {
        toast.error(
          "Unable to establish a database connection for this project.",
        );
        return null;
      }
    ...
    <>
      {isSnippet ? (
        <QueryBlock
          ...
          onRemoveChart={() =>
            onRemoveChart({ metric: { key: item.attribute } })
          }
