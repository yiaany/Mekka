...
export const generateToolRoutes = (ref?: string, project?: Project): Route[] => {
  ...
  return [
    ...
    route.key === 'editor' ? STUDIO_FEATURES.tableEditor : STUDIO_FEATURES.sqlEditor
  )
}
...
export const generateProductRoutes = (
  ...
  const databaseEnabled = STUDIO_FEATURES.database && (features?.database ?? true)
  const authEnabled = STUDIO_FEATURES.auth && (features?.auth ?? true)
  const edgeFunctionsEnabled = STUDIO_FEATURES.edgeFunctions && (features?.edgeFunctions ?? true)
  const storageEnabled = STUDIO_FEATURES.storage && (features?.storage ?? true)
  const realtimeEnabled = STUDIO_FEATURES.realtime && (features?.realtime ?? true)
...
export const generateOtherRoutes = (
  ...
  const advisorsEnabled = STUDIO_FEATURES.advisors
  const reportsEnabled = STUDIO_FEATURES.observability && (features?.showReports ?? true)
  const logsEnabled = STUDIO_FEATURES.logs && (features?.showLogs ?? true)
...
export const generateSettingsRoutes = (ref?: string): Route[] => {
  if (!STUDIO_FEATURES.settings) return []
