...
const MekkaLogo = () => (
  <img src={`${BASE_PATH}/img/mekka-logo.svg`} alt={STUDIO_BRAND.name} className="h-7 w-7" />
)
  ...
  if (inboundFlow && focusProvider) {
    return (
      <InterstitialLayout
        ...
        title={destination ? `Continue to ${destination.displayName}` : `${verb} to ${STUDIO_BRAND.name}`}
        description={
          destination
            ? `${verb} to ${STUDIO_BRAND.name} using your ${focusProvider.displayName} account`
            ...
            <div className="flex items-center grow shrink-0 lg:grow-0">
              <div className="flex items-center justify-between w-full md:w-auto">
                <Link href="/organizations">
                  <img
                    src={`${BASE_PATH}/img/mekka-logo.svg`}
                    alt={STUDIO_BRAND.name}
                    className={largeLogo ? 'h-[48px] w-[48px]' : 'h-[24px] w-[24px]'}
                  />
                </Link>
              </div>
            </div>
