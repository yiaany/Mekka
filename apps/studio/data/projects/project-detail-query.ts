...
export async function getProjectDetail(
  ...
  if (data?.is_hibernating && !skipWake) {
    ...
    const { error: errorWaking, data: wakeResponse } = await post(
    ...
    if (wakeResponse) {
      ...
      queryClient?.setQueryData<ReadReplicasData>(
        replicaKeys.list(ref),
        (old) => {
          if (!old) return old;
            ...
            if (db.identifier === ref) {
              return {
                ...
                connection_string_read_only:
                  wakeResponse.connection_string_read_only,
              };
            }
            return db;
          });
...
export const useProjectDetailQuery = <TData = ProjectDetailData>(
  ...
  return useQuery<ProjectDetailData, ProjectDetailError, TData>({
    ...
    queryFn: ({ signal }) =>
      getProjectDetail({ ref }, signal, undefined, queryClient),
    ...
    refetchInterval: (query) => {
      ...
      if (
        status === "COMING_UP" ||
        status === "UNKNOWN" ||
        (IS_PLATFORM && !isValidConnString(connectionString))
      ) {
...
export const useSetProjectPostgrestStatus = () => {
  ...
  const setProjectPostgrestStatus = (
    ref: Project["ref"],
