...
import { getLiveTupleEstimateKey } from "@/data/database/keys";
...
const getBucketsWithLargestSizeLimit = async ({
  ...
  const { result } = await executeSql<
    { id: string; name: string; file_size_limit: number }[]
  >({
    projectRef,
    connectionString,
    queryKey: key,
...
export const useLargestBucketSizeLimitsCheck = ({
  ...
  const { data: estimatedRows, isPending: isEstimatePending } = useQuery<
    number | undefined
  >({
    queryKey: [...estimateKey, projectRef, connectionString],
    queryFn: () => getBucketNumberEstimate({ projectRef, connectionString }),
    enabled: !!projectRef && !!connectionString,
  });

  const bucketLimitsKey = sqlKeys.query(projectRef, [
    "buckets-with-largest-size-limit",
  ]);

  const fetchLargestBucketLimits = useCallback(
    () =>
      queryClient.fetchQuery({
        queryKey: [...bucketLimitsKey, projectRef, connectionString],
        queryFn: () =>
          getBucketsWithLargestSizeLimit({ projectRef, connectionString }),
      }),
  ...
  return {
    runCondition:
      ...
      estimatedRows <= THRESHOLD_FOR_AUTO_QUERYING_BUCKET_LIMITS
        ? "auto"
        : "confirm",
