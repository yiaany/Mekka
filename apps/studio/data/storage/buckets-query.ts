...
import {
  ...
  getBucketNumberEstimateKey,
} from "./buckets-max-size-limit-query";
...
export type BucketsListPaginatedVariables = BucketsVariables &
  BucketListPagination;
...
export type BucketsWithPaginationData = Awaited<
  ReturnType<typeof getBucketsPaginated>
>;
...
export const useBucketQuery = <TData = BucketData>(
  { projectRef, bucketId }: GetBucketParams,
  {
    enabled = true,
    ...options
  }: UseCustomQueryOptions<BucketData, BucketsError, TData>,
) => {
...
export const useBucketNumberEstimateQuery = (
  { projectRef }: BucketsVariables,
  {
    enabled = true,
    ...options
  }: UseCustomQueryOptions<number | undefined, ResponseError> = {},
) => {
  const { data: project } = useSelectedProjectQuery();
  const connectionString = project?.connectionString;

  return useQuery<number | undefined, ResponseError>({
    queryKey: [...getBucketNumberEstimateKey(projectRef), connectionString],
    queryFn: () =>
      getBucketNumberEstimate({
        projectRef,
        connectionString,
 ...
 * from remote by name.
 */
export const useBucketInfoQueryPreferCached = (
  bucketId?: string,
  projectRef?: string,
) => {
  ...
  const cachedBucketInfo = useMemo(() => {
    ...
    const bucketsPages = queryClient.getQueryData(
      storageKeys.bucketsList(projectRef),
    ) as InfiniteData<Bucket[]> | undefined;
