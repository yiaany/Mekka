...
const getPreferredOrderByColumns = (
  ...
  cursorPaginationEligible: string[][];
  cursorPaginationNonEligible: string[];
} => {
  ...
  const eligibleColumnsForSorting = table.columns.filter(
    (x) => !x.dataType.includes("json"),
  );
  cursorPaginationNonEligible.push(
    ...eligibleColumnsForSorting.map((col) => col.name),
  );
...
export const getAllTableRowsSql = ({
  ...
  if (sorts.length === 0) {
    if (cursorPaginationEligible.length > 0) {
      ...
      queryChains = queryChains.order(
        table.name,
        cursorPaginationNonEligible[0],
      );
      if (hasCtid) {
...
export const useTableRowsQuery = <TData = TableRowsData>(
  {
    ...
    tableId,
    ...args
  }: Omit<TableRowsVariables, "queryClient" | "connectionString">,
  {
    enabled = true,
    ...options
  }: UseCustomQueryOptions<TableRowsData, TableRowsError, TData> = {},
) => {
  const { connectionString, identifier: readReplicaIdentifier } =
  ...
  return useQuery<TableRowsData, TableRowsError, TData>({
    ...
    queryFn: ({ signal, client }) =>
      getTableRows(
        { queryClient: client, projectRef, connectionString, tableId, ...args, scoped },
        signal,
      ),
    enabled:
      enabled &&
...
export function prefetchTableRows(
  ...
  return client.fetchQuery({
    ...
    queryFn: ({ signal, client: queryClient }) =>
      getTableRows(
        { queryClient, projectRef, connectionString, tableId, ...args },
        signal,
      ),
  });
}
