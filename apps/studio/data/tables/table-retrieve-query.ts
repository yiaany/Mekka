...
import {
  ...
  scopedIntrospectionReady,
} from "@/data/scoped-introspection";
...
export async function getTable(
  ...
  const { sql, zod } = pgMeta.tables.retrieve({
    name,
    schema,
...
export type RetrievedTableColumn = NonNullable<
  RetrieveTableResult["columns"]
>[number];
...
export const useTableQuery = <TData = RetrieveTableResult>(
  ...
  return useQuery<RetrieveTableResult, RetrieveTableError, TData>({
    ...
    queryFn: ({ signal }) =>
      getTable({ projectRef, connectionString, name, schema }, signal),
