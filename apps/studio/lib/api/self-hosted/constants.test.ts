...
describe('api/self-hosted/constants', () => {
  ...
  describe('ENCRYPTION_KEY', () => {
    ...
    it('should not provide a default encryption key', async () => {
      vi.stubEnv('PG_META_CRYPTO_KEY', '')
      const { ENCRYPTION_KEY } = await import('./constants')
      expect(ENCRYPTION_KEY).toBeUndefined()
    })
  })
  ...
  describe('POSTGRES_DATABASE', () => {
    ...
    it('should not provide a default password', async () => {
  ...
  describe('POSTGRES_PASSWORD', () => {
    ...
    it('should default to postgres', async () => {
      ...
      const { POSTGRES_PASSWORD } = await import('./constants')
      expect(POSTGRES_PASSWORD).toBeUndefined()
    })
  })
  ...
  describe('AUTH_JWT_SECRET', () => {
    ...
    it('should not provide a default JWT secret', async () => {
      vi.stubEnv('AUTH_JWT_SECRET', '')
      const { AUTH_JWT_SECRET } = await import('./constants')
      expect(AUTH_JWT_SECRET).toBeUndefined()
    })
  })
})
