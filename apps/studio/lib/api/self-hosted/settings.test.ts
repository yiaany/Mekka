...
vi.hoisted(() => {
  process.env.AUTH_JWT_SECRET = 'test-jwt-secret-with-at-least-32-characters'
})
...
describe('api/self-hosted/settings', () => {
  ...
  describe('getProjectSettings', () => {
    ...
    it('should reject a missing JWT secret', async () => {
      vi.unstubAllEnvs()

      vi.resetModules()
      const { getProjectSettings: getSettings } = await import('./settings')
      expect(() => getSettings()).toThrow('AUTH_JWT_SECRET must be configured for Mekka Studio')
    })
