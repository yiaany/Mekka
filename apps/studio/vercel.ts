...
export const tanstackFunctionIncludeFiles =
  '{dist/server/**,../../node_modules/**/libpg-query/wasm/libpg-query.wasm}'
...
function applyBasePath(r: StudioRedirect): VercelRedirect {
...
function buildRedirects(): VercelRedirect[] {
  ...
  const basePathBounce: VercelRedirect[] = basePath
  ...
  return {
    ...
    functions: {
      'api/server.js': {
        ...
        //
        // Match the installed package's real WASM file instead of one package
        // manager's store layout. Bun, pnpm and npm use different paths, but
        // all retain this package-relative location after dependency linking.
        includeFiles: tanstackFunctionIncludeFiles,
      },
    },
  }
}
